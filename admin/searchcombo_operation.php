...
if($_REQUEST['op']=='delete')
{
	...
	$id_delete=explode(",",$id_delete);  
 	if($id_delete!="")
	{	
	  	for($i=0;$i<=count($id_delete)-1;$i++)
	  	{
			//echo $id_delete[$i]." ";
	    	$sql_delete="delete from contact_mst where contact_id= '$id_delete[$i]' ";
			//echo $sql_delete;
...
if($_GET['op']=='search')
{
	...
	{
		$where.=" and source_id like '%".$_REQUEST['source']."%'";
	}
	if(isset($_REQUEST['category']) && $_REQUEST['category']!="")	
	{
		$where.=" and category_id like '%".$_REQUEST['category']."%'";
	}
