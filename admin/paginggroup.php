<?php
		...
		$query="select count(group_id) from groups where group_name like '%".$search."%'";
...
?>    
		...
		<?php
			...
			if(isset($_GET['order']))
			{
				$sql1 ="select a.group_id,a.group_name from (select group_id,group_name from groups where group_name like '%".$search."%' limit $offset,$rec_limit) a order by group_name ".$_GET['order'] ;
			}
			else
			{
				$sql1="select * from groups where group_name like '%".$search."%' limit $offset,$rec_limit" ;
			}
		$result1=mysql_query($sql1);
		$total_id="0";
		while($row1=mysql_fetch_array($result1))
		{
			$total_id.=",".$row1['group_id'];
		}
