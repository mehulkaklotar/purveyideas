<?php
		...
		$query="select count(source_id) from source_mst where source_name like '%".$search."%'";
...
?>    
   ...
   <?php 
		...
		if(isset($_GET['search']))
			...
			if(isset($_GET['order']))
			{
				$sql1 ="select a.source_id,a.source_name from (select source_id,source_name from source_mst where source_name like '%".$search."%' limit $offset,$rec_limit) a order by source_name ".$_GET['order'] ;	
			}
			else
			{
				$sql1 ="select * from source_mst where source_name like '%".$search."%'  limit $offset,$rec_limit";
        ...
        <?php include("source_content.php"); ?>
		<?php
        }
		else
		{?>
