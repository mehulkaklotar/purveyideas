<?php
		...
		$query="select count(sms_id) from sms_template where sms_title like '%".$search."%'";
...
?>    
		...
		if(isset($_REQUEST['search']))
			...
			{
				$sql1 ="select a.sms_id,a.sms_title,a.sms_text from (select sms_id,sms_title,sms_text from sms_template where sms_title like '%".$search."%' limit $offset,$rec_limit) a order by ".$_REQUEST['field']." ".$_REQUEST['order'];
			}
			else
			{
				$sql1 ="select sms_id,sms_title,sms_text from sms_template where sms_title like '%".$search."%' limit $offset,$rec_limit";
        ...
        <?php include("smstemp_content.php"); ?>
