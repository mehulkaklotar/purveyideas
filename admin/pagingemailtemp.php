<?php
		...
		$query="select count(email_id) from email_template where email_title like '%".$search."%'";
...
?>    
		...
		if(isset($_REQUEST['search']))
			...
			{
				$sql1 ="select a.email_id,a.email_title,a.email_text from (select email_id,email_title,email_text from email_template where email_title like '%".$search."%' limit $offset,$rec_limit) a order by ".$_REQUEST['field']." ".$_REQUEST['order'];
			}
			else
			{
				$sql1 ="select email_id,email_title,email_text from email_template where email_title like '%".$search."%' limit $offset,$rec_limit";
        ...
        <?php include("emailtemp_content.php"); ?>
