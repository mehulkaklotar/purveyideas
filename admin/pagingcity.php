<?php
		...
		$query="select count(city_id) from city_mst where city_name like '%".$search."%'";
...
?>    
		...
		if(isset($_GET['search']))
			...
			if(isset($_GET['order']))
			{						
				$sql1="select a.city_id,a.city_name,a.state_name,a.state_id from (select c1.city_id,c1.city_name,c1.state_id,s1.state_name from city_mst c1,state_mst s1 where c1.state_id=s1.state_id and c1.city_name like '%".$search."%' limit $offset,$rec_limit) a order by ".$_GET['field']." ".$_GET['order'];
							
			}
			else
			{
				$sql1 ="select * from city_mst where city_name like '%".$search."%' limit $offset,$rec_limit";
