<?php
		...
		$query="select count(news_id) from newsletter where news_sub like '%".$search."%'";
...
?>    
		...
		if(isset($_REQUEST['search']))
			...
			{
				$sql1 ="select a.news_id,a.news_sub,a.news_content,a.date_added from (select news_id,news_sub,news_content,date_added from newsletter where news_sub like '%".$search."%' limit $offset,$rec_limit) a order by ".$_REQUEST['field']." ".$_REQUEST['order'];
			}
			else
			{
				$sql1 ="select * from newsletter where news_sub like '%".$search."%' limit $offset,$rec_limit";
