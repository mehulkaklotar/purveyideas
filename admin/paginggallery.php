<?php
		...
		$query="select count(id) from gallery where caption like '%".$search."%'";
...
?>    
		...
		if(isset($_GET['search']))
			...
			if(isset($_GET['order']))
			{
				$sql1 ="select a.id,a.caption,a.name from (select id,caption,name from gallery where caption like '%".$search."%' limit $offset,$rec_limit) a order by ".$_GET['field']." ".$_GET['order'];
			}
			else
			{
				$sql1 ="select * from gallery where caption like '%".$search."%' limit $offset,$rec_limit";
